...
MotionManager::MotionManager() : cyberdog_decision::CascadeManager("motion_manager", "") {
...
CallbackReturn_T MotionManager::on_configure(const rclcpp_lifecycle::State &) {
  ...
  topic_name_map_.insert(
      std::pair<std::string, std::string>("rc_topic", get_parameter("topic_name_rc").as_string()));
  ...
  callback_group_service = this->create_callback_group(rclcpp::CallbackGroupType::Reentrant);

  mode_server_ =
      std::make_unique<cyberdog_utils::SimpleActionServer<ChangeMode_T, LifecycleNode_T>>(
          this->shared_from_this(), "checkout_mode",
          std::bind(&MotionManager::checkout_mode, this));

  gait_server_ =
      std::make_unique<cyberdog_utils::SimpleActionServer<ChangeGait_T, LifecycleNode_T>>(
          this->shared_from_this(), "checkout_gait",
          std::bind(&MotionManager::checkout_gait, this));

  monorder_server_ =
      std::make_unique<cyberdog_utils::SimpleActionServer<ExtMonOrder_T, LifecycleNode_T>>(
          this->shared_from_this(), "exe_monorder",
          std::bind(&MotionManager::mon_order_exec, this));

  gait_pub_ = this->create_publisher<Gait_T>("gait_out", rclcpp::SystemDefaultsQoS());

  control_state_pub_ =
      this->create_publisher<ControlState_T>("status_out", rclcpp::SystemDefaultsQoS());

  odom_pub_ =
      this->create_publisher<nav_msgs::msg::Odometry>("odom_out", rclcpp::SystemDefaultsQoS());

  cmd_pub_ = this->create_publisher<SE3VelocityCMD_T>("cmd_out", rclcpp::SystemDefaultsQoS());

  casual_service_client_ = this->create_client<automation_msgs::srv::NavMode>("nav_mode");
  ...
  velocity_sub_ = this->create_subscription<SE3VelocityCMD_T>(
      ...
      std::bind(&MotionManager::velocity_cmd_callback, this, std::placeholders::_1));

  ob_detect_sub_ = this->create_subscription<Around_T>(
      "ObstacleDetection", rclcpp::SystemDefaultsQoS(),
      std::bind(&MotionManager::ob_detection_callback, this, std::placeholders::_1));
  ...
  cau_sub_ = this->create_subscription<NavCaution_T>(
      ...
      std::bind(&MotionManager::caution_callback, this, std::placeholders::_1));

  control_state_sub_ = this->create_subscription<ControlState_T>(
      "status_out", rclcpp::SystemDefaultsQoS(),
      std::bind(&MotionManager::control_state_callback, this, std::placeholders::_1));
  ...
  tf_broadcaster =
      std::make_unique<tf2_ros::TransformBroadcaster>(this, rclcpp::SystemDefaultsQoS());

  motion_in_ = std::make_unique<lcm::LCM>(this->get_lcm_url(
      std::string("239.255.76.67"), port_recv_from_motion_, ttl_recv_from_motion_));
  motion_out_ = std::make_unique<lcm::LCM>(
      this->get_lcm_url(std::string("239.255.76.67"), port_send_to_motion_, ttl_send_to_motion_));
  state_es_in_ = std::make_unique<lcm::LCM>(
      this->get_lcm_url(std::string("239.255.76.67"), port_from_odom_, ttl_from_odom_));

  motion_in_->subscribe("exec_response", &MotionManager::control_lcm_collection, this);
  state_es_in_->subscribe("state_estimator", &MotionManager::statees_lcm_collection, this);
...
void MotionManager::checkout_mode() {
  ...
  auto gait_before = (robot_control_state_.gaitstamped.gait == Gait_T::GAIT_PASSIVE)
                         ? Gait_T::GAIT_KNEEL
                         : robot_control_state_.gaitstamped.gait;
  ...
  auto TAG_MODE = std::string("[Mode_Check]-[") + mode_str + std::string("] ");

  message_info(
      TAG_MODE + std::string("Got mode check request, goal mode is ") +
      mode_label_[next_mode.control_mode]);

  if (!mode_server_ || !mode_server_->is_server_active()) {
    message_info(TAG_MODE + std::string("Mode Server is not active"));
...
#ifndef DEBUG_ALL
  if (check_time_update(goal->modestamped.timestamp, current_mode.timestamp)) {
    new_request = true;
  } else {
    message_warn(
        TAG_MODE + std::string("Warning, old timestamp, mode ") +
        mode_label_[next_mode.control_mode] + std::string(" will not be executed"));
  ...
  if (next_mode.control_mode <= Mode_T::MODE_SEMI) {
    ...
    if (submode_map_.find(std::pair<uint8_t, uint8_t>(
            next_mode.control_mode, next_mode.mode_type)) == submode_map_.end()) {
      result->err_code = ModeRes_T::UNKNOWN;
      new_request = false;
    }
  }
  ...
  if (automation_manager_node_->get_current_state().id() ==  // NOLINT
    ...
    while (automation_manager_node_->chainnodes_state_ != cyberdog_decision::ALL_DEACTIVE &&
      ...
      if (this->get_clock()->now() - current_stamp >= std::chrono::seconds(timeout_manager_)) {
        message_warn(
            TAG_MODE + std::string("Subnodes deactivate failed, some nodes running back end"));
        break;
      }
    }
  ...
  if (robot_control_state_.safety.status == Safety_T::LOW_BTR) {
    if (next_mode.control_mode >= Mode_T::MODE_SEMI) {
      ...
      message_warn(TAG_MODE + std::string("Battery low, reject high consumption mode"));
      new_request = false;
    }
  }
  ...
  while (rclcpp::ok() && new_request) {
    if (mode_server_->is_cancel_requested()) {
      message_info(TAG_MODE + std::string("Mode Server is canceled"));
      result->err_code = ModeRes_T::CANCELED;
      new_request = false;
      if (next_mode.control_mode >= Mode_T::MODE_SEMI) {
        message_info(TAG_MODE + std::string("Auto recycle automation nodes"));
        if (automation_manager_node_->get_current_state().id() ==  // NOLINT
                lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE ||
    ...
    if (mode_server_->is_preempt_requested()) {
      ...
      if (pending_mode.control_mode == next_mode.control_mode &&
        ...
        message_info(TAG_MODE + std::string("Pending goal is same with current checking, reject"));
        mode_server_->terminate_pending_goal();
      } else {
        message_info(
            TAG_MODE + std::string("New mode checking req received, terminate current now"));
    ...
    if (new_request) {
      switch (next_mode.control_mode) {
        ...
        case Mode_T::MODE_MANUAL: {
          ...
          bool condition_pre =
              next_mode.control_mode == Mode_T::MODE_DEFAULT ? condition_default : condition_manual;
            ...
            if (check_inside) {
              if (this->get_clock()->now() - current_time <=
                ...
                if (inter_changed_ == cyberdog_decision::SUCCEED) {
                  message_info(
                      TAG_MODE + std::string("Auto ") + gait_label_[mode_trig_gait.gait] +
                      std::string(" succeed"));
                  new_request = false;
                ...
                message_warn(TAG_MODE + std::string("Timeout, canceling mode checking"));
        ...
        case Mode_T::MODE_SEMI: {
          message_info(
              TAG_MODE + std::string("Currently not support mode ") +
              mode_label_[next_mode.control_mode]);
          result->err_code = ModeRes_T::UNKNOWN;
          new_request = false;
          break;
        ...
        case Mode_T::MODE_TRACK: {
          if (monorder_server_->is_running()) {
            message_warn(TAG_MODE + std::string("Order is executing, reject other mode checking"));
            result->err_code = ModeRes_T::REJECT;
            new_request = false;
            break;
          }
          if (!check_submode) {
            if (automation_manager_node_->chainnodes_state_ == cyberdog_decision::ALL_ACTIVE) {
              message_info(TAG_MODE + std::string("Subnodes are active now"));
              check_submode = true;
            } else if (
                automation_manager_node_->get_current_state().id() !=  // NOLINT
                lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE) {
            ...
            if (check_inside) {
              if (this->get_clock()->now() - current_time <=
                ...
                if (inter_changed_ != cyberdog_decision::DEFAULT) {
                  if (inter_changed_ == cyberdog_decision::SUCCEED) {
                    message_info(
                        TAG_MODE + std::string("Check submode ") +
                        submode_label_[next_mode.mode_type] + std::string(" succeed"));
                    new_request = false;
                  } else if (inter_changed_ == cyberdog_decision::FAILED) {
                    message_info(
                        TAG_MODE + std::string("Check submode ") +
                        submode_label_[next_mode.mode_type] + std::string(" failed"));
                    new_request = false;
                    result->err_code = ModeRes_T::FAILED;
                    result->err_state = feedback->current_state;
                  }
              ...
              auto request = std::make_unique<automation_msgs::srv::NavMode::Request>();
              request->sub_mode = submode_map_[std::pair<uint8_t, uint8_t>(
                  next_mode.control_mode, next_mode.mode_type)];
              auto client_callback = [&](rclcpp::Client<SubMode_T>::SharedFuture future) {
              ...
              casual_service_client_->async_send_request(std::move(request), client_callback);
              message_info(TAG_MODE + std::string("Sent request, waiting for response"));
              check_inside = true;
            }
            feedback->timestamp = this->get_clock()->now();
  ...
  if (result->err_code == ModeRes_T::NORMAL) {
    message_info(
        TAG_MODE + std::string("Check mode ") + mode_label_[next_mode.control_mode] +
        std::string(", submode ") + submode_label_[next_mode.mode_type] + std::string(" succeed"));
    ...
    if (gait_before != robot_control_state_.gaitstamped.gait &&
        result->err_code != ModeRes_T::AVOID_PREEMPT) {
      message_warn(TAG_MODE + std::string("Mode check failed, recovery gait before"));
      Gait_T recover_gait;
    ...
    message_warn(
        TAG_MODE + std::string("Check mode ") + mode_label_[next_mode.control_mode] +
        std::string(", submode ") + submode_label_[next_mode.mode_type] +
        std::string(" failed. Error code is ") + mode_errcode_label_[result->err_code] +
        std::string(", error state is ") + mode_errstate_label_[result->err_state]);
  }
  mode_server_->succeeded_current(std::move(result));
}
...
void MotionManager::checkout_gait() {
  ...
  auto TAG_GAIT = std::string("[Gait_Check]-[") + motivation_str + std::string("]-[") + gait_str +
                  std::string("] ");

  message_info(
      TAG_GAIT + std::string("Got gait check request, goal gait is ") +
      gait_label_[goal_gait.gait]);

  if (!gait_server_ || !gait_server_->is_server_active()) {
    message_info(TAG_GAIT + std::string("Gait Server is not active"));
...
#ifndef DEBUG_ALL
  if (check_time_update(goal->gaitstamped.timestamp, robot_control_state_.gaitstamped.timestamp)) {
    new_request = true;
  } else {
    message_warn(TAG_GAIT + std::string("Warning, old timestamp, gait will not be executed"));
  ...
  if (running_mode.control_mode <= Mode_T::MODE_LOCK && goal_gait.gait != Gait_T::GAIT_PASSIVE &&
      current_motivation != cyberdog_utils::MODE_TRIG) {
    message_warn(
        TAG_GAIT + std::string("Warning, current mode ") + mode_label_[running_mode.control_mode] +
        std::string(" do not support checking gait, please check "
  ...
  if (gait_motivation_map_.find(current_motivation) == gait_motivation_map_.end()) {
    result->err_code = GaitRes_T::UNAVAILABLE;
    message_warn(TAG_GAIT + std::string("Unavailable motivation"));
    new_request = false;
  }
  ...
  if (robot_control_state_.safety.status == Safety_T::LOW_BTR) {
    if (goal_gait.gait <= Gait_T::GAIT_DEFAULT && goal_gait.gait >= Gait_T::GAIT_BOUND) {
      result->err_code = GaitRes_T::UNAVAILABLE;
      message_warn(TAG_GAIT + std::string("Battery low, reject dangerous gait"));
      new_request = false;
    }
  }
  ...
  if (new_request) {
    ...
    if (gait_list.back().gait == current_gait) {
      message_info(
          TAG_GAIT + std::string("Gait running currently is already ") + gait_label_[current_gait]);
      result->succeed = true;
    ...
    while (rclcpp::ok() && gait_list.size() > 0) {
      if ((robot_control_state_.modestamped != running_mode) &&
          current_motivation != cyberdog_utils::MODE_TRIG) {
        message_warn(TAG_GAIT + std::string("Mode changed, gait checking will be interrupted"));
        result->err_code = GaitRes_T::INTERRUPTED;
        gait_list.clear();
        break;
      }

      if (this->get_clock()->now() - current_time >= std::chrono::seconds(timeout_gait_ * 2)) {
        message_error(
            TAG_GAIT + std::string("Stuck for ") + std::to_string(timeout_gait_ * 2) +
      ...
      if (gait_server_->is_cancel_requested()) {
        message_info(TAG_GAIT + std::string("Gait Server is canceled"));
        result->err_code = ModeRes_T::CANCELED;
        gait_list.clear();
        break;
      }
      ...
      if (gait_server_->is_preempt_requested()) {
        auto pending_motivation = gait_server_->get_pending_goal()->motivation;
        auto pending_gait = gait_server_->get_pending_goal()->gaitstamped;
        if (pending_motivation < current_motivation ||
            (pending_motivation == current_motivation && goal_gait != pending_gait)) {
          message_info(TAG_GAIT + std::string("High priority preempt request, disable current"));
          ...
          auto motivation_str =
              gait_motivation_map_.find(pending_motivation) != gait_motivation_map_.end()
                  ? gait_motivation_map_[pending_motivation]
                  : std::to_string(pending_motivation);
          message_info(
              TAG_GAIT + std::string("Low priority preempt [") + motivation_str +
      ...
      if (current_gait != Gait_T::GAIT_TRANS || gait_cached_.gait != current_sending_) {
        if (gait_cached_.gait != current_sending_ && current_sending_ != Gait_T::GAIT_TRANS) {
          message_info(
              TAG_GAIT + std::string("Gait checking current is not what client request, add [") +
        ...
        switch (gait_list.back().gait) {
          case Gait_T::GAIT_PASSIVE: {
            message_info(
                TAG_GAIT + std::string("Check ") + gait_label_[gait_list.back().gait] +
                std::string(" directly"));
          ...
          case Gait_T::GAIT_KNEEL: {
            if (current_gait == Gait_T::GAIT_PASSIVE) {
              message_warn(std::string(
                  TAG_GAIT + std::string("Future condition met, no need to check ") +
                  gait_label_[gait_list.back().gait] + std::string(". Exit")));
              gait_list.clear();
              break;
            } else if (current_gait == Gait_T::GAIT_STAND_R) {
              message_info(std::string(
                  TAG_GAIT + std::string("Condition met, check ") +
                  gait_label_[gait_list.back().gait] + std::string(" directly")));
              gait_to_pub.gait = gait_list.back().gait;
            } else {
              Gait_T pre_gait;
              pre_gait.gait = Gait_T::GAIT_STAND_R;
              message_info(
                  TAG_GAIT + std::string("Condition not met, check to ") +
          ...
          case Gait_T::GAIT_STAND_R: {
            if (current_gait == Gait_T::GAIT_PASSIVE || current_gait == Gait_T::GAIT_KNEEL ||
              ...
              gait_to_pub.gait = gait_list.back().gait;
                ...
                current_gait > Gait_T::GAIT_STAND_B &&  // NOLINT
          ...
          default: {
            message_info(TAG_GAIT + std::string("Unknown gait, reject"));
      ...
      auto current_time = this->get_clock()->now();
      while (rclcpp::ok() && gait_to_pub.gait != Gait_T::GAIT_DEFAULT) {
        if ((robot_control_state_.modestamped.control_mode != running_mode.control_mode ||
             (robot_control_state_.modestamped.control_mode == running_mode.control_mode &&
              robot_control_state_.modestamped.mode_type != running_mode.mode_type)) &&
            current_motivation != cyberdog_utils::MODE_TRIG) {
          message_warn(TAG_GAIT + std::string("Mode changed, gait checking will be canceled"));
        ...
        if (gait_server_->is_preempt_requested()) {
          auto pending_motivation = gait_server_->get_pending_goal()->motivation;
          auto pending_gait = gait_server_->get_pending_goal()->gaitstamped.gait;
          if (pending_motivation < current_motivation ||
        ...
        if (this->get_clock()->now() - current_time >= std::chrono::seconds(timeout_gait_)) {
          message_info(
              TAG_GAIT + std::string("Timeout, cancel check ") + gait_label_[gait_to_pub.gait]);
          result->err_gait = gait_list.back();
          result->err_code = GaitRes_T::TIME_OUT;
          gait_list.clear();
          break;
        }
        current_gait = (gait_to_pub.gait > Gait_T::GAIT_STAND_B)
                           ? gait_cached_.gait
                           : robot_control_state_.gaitstamped.gait;
        ...
        if (current_gait == gait_to_pub.gait && current_gait == gait_cached_.gait) {
          message_info(
              TAG_GAIT + std::string("Gait ") + gait_label_[current_gait] +
  ...
  if (result->err_code == GaitRes_T::NORMAL) {
    ...
    result->succeed = false;
    message_warn(
        TAG_GAIT + std::string("Check gait execute failed. Error code is ") +
        gait_errcode_label_[result->err_code] + std::string(", error gait is ") +
...
void MotionManager::mon_order_exec() {
  ...
  auto order_str = (order_label_.count(goal_order.id) == 0) ? std::to_string(goal_order.id)
                                                            : order_label_[goal_order.id];
  auto TAG_ORDER = std::string("[Order_Exec]-[") + order_str + std::string("] ");

  message_info(TAG_ORDER + std::string("Got order execution request"));

  if (!monorder_server_ || !monorder_server_->is_server_active()) {
    message_info(TAG_ORDER + std::string("Mon Order Server is not active"));
...
#ifndef DEBUG_ALL
  if (check_time_update(goal_order.timestamp, robot_control_state_.orderstamped.timestamp)) {
    new_request = true;
  } else {
    message_info(
        TAG_ORDER + std::string("Warning, old timestamp, order ") + std::to_string(goal_order.id) +
  ...
  if (robot_control_state_.modestamped.control_mode != Mode_T::MODE_MANUAL) {
    message_info(
        TAG_ORDER + std::string("Order will not execute in current mode [") +
  ...
  // Battery checking
  if (robot_control_state_.safety.status == Safety_T::LOW_BTR) {
    message_info(TAG_ORDER + std::string("Battery low, reject all action request"));
    new_request = false;
    result->err_code = MonOrderRes_T::UNAVAILABLE;
  }
  ...
  if (new_request) {
    ...
    switch (goal_order.id) {
      ...
      case MonOrder_T::MONO_ORDER_TURN_AROUND: {
        if (gait_cached_.gait != order_to_gait_[goal_order.id]) {
          ...
          auto check_result =
              inter_check_gait_sync(gait_to_use, timeout_gait_, cyberdog_utils::ORDER_REQ);
          if (!check_result) {
            message_info(TAG_ORDER + std::string("Gait check failed"));
            new_request = false;
            result->err_code = MonOrderRes_T::FAILED;
          }
        } else {
          message_info(TAG_ORDER + std::string("Robot is already in current state"));
        }
        break;
      }
      ...
      case MonOrder_T::MONO_ORDER_SHOW: {
        auto order_file = local_params_dir + std::string("/orders/") + order_label_[goal_order.id] +
                          std::string(".toml");
        if (!std::experimental::filesystem::exists(order_file)) {
          ...
          if (!inter_check_gait_sync(gait_to_use, timeout_gait_, cyberdog_utils::ORDER_REQ)) {
            result->err_code = MonOrderRes_T::FAILED;
            message_info(TAG_ORDER + std::string("Recovery stand failed, canceling"));
            new_request = false;
          }
      ...
      default: {
        message_info(
            TAG_ORDER + std::string("Unknown order id [") + std::to_string(goal_order.id) +
            std::string("]"));
        new_request = false;
    ...
    auto current_time = this->get_clock()->now();
    auto cons_speed = goal_order.id == MonOrder_T::MONO_ORDER_TURN_AROUND ? cons_speed_a_normal_
                                                                          : cons_speed_l_normal_;
    auto running_time = static_cast<int64_t>(std::abs(std::ceil(goal_order.para / cons_speed)));
    // running cycle
    while (rclcpp::ok() && new_request) {
      if (this->get_clock()->now() - current_time >= std::chrono::seconds(timeout_order_ / 4)) {
        message_warn(
            TAG_ORDER + std::string("Stuck for ") + std::to_string(timeout_order_ / 4) +
            std::string(" seconds. Canceling"));
      ...
      if (robot_control_state_.modestamped != running_mode) {
        message_warn(TAG_ORDER + std::string("Mode changed, order executing will be canceled"));
      ...
      if ((gait_cached_.gait != gait_to_use.gait) && interruptible) {
        message_warn(
            TAG_ORDER + std::string("Gait interruption occured, order executing will be canceled"));
      ...
      if ((vcmd_bak_ != ext_velocity_cmd_) && interruptible) {
        message_warn(
            TAG_ORDER + std::string("VCMD interruption occured, order executing will be canceled"));
        result->err_code = MonOrderRes_T::INTERRUPTED;
        new_request = false;
        break;
      }
      ...
      if (monorder_server_->is_cancel_requested()) {
        message_info(TAG_ORDER + std::string("Order request is canceled"));
        result->err_code = ModeRes_T::CANCELED;
        new_request = false;
        break;
      }
      ...
      if (monorder_server_->is_preempt_requested()) {
        message_info(TAG_ORDER + std::string("A preempt order received"));
        if (interruptible) {
          monorder_server_->terminate_current();
          result->err_code = MonOrderRes_T::INTERRUPTED;
          new_request = false;
          message_info(
              TAG_ORDER + std::string("Order could be interrupted, terminate current order"));
          break;
        } else {
          monorder_server_->terminate_pending_goal();
          message_info(
              TAG_ORDER + std::string("Order could NOT be interrupted, terminate pending order"));
        }
      }
      ...
      if (!need_order_list) {  // gait-like order
        ...
        if (goal_order.para == 0.0) {
          ...
          if (cost_time > std::chrono::seconds(running_time)) {
            message_info(TAG_ORDER + std::string("Movement order is complete"));
            new_request = false;
            break;
            ...
            velocity_order->velocity.linear_x =
                (goal_order.id == MonOrder_T::MONO_ORDER_STEP_BACK) ? -cons_speed_l_normal_ : 0;
            velocity_order->velocity.angular_z =
                (goal_order.id == MonOrder_T::MONO_ORDER_TURN_AROUND) ? cons_speed_a_normal_ : 0;
        ...
        auto condition_timeout =
            this->get_clock()->now() - current_time >= std::chrono::seconds(timeout_order_);
  ...
  if (result->err_code == MonOrderRes_T::NORMAL) {
    message_info(TAG_ORDER + std::string("Order execution is succeed"));
    robot_control_state_.orderstamped.timestamp = this->get_clock()->now();
    result->succeed = true;
  } else {
    message_info(
        TAG_ORDER + std::string("Order execution is failed, error code is ") +
...
void MotionManager::velocity_cmd_callback(const SE3VelocityCMD_T::SharedPtr msg) {
  auto current_mode = robot_control_state_.modestamped;
  ...
  switch (current_mode.control_mode) {
    ...
    default: {
      message_warn(TAG_VCMD + std::string("Not support velocity command right now"));
      break;
    }
  }
}

void MotionManager::ob_detection_callback(const Around_T::SharedPtr msg) { obstacle_data_ = *msg; }
...
void MotionManager::paras_callback(const Parameters_T::SharedPtr msg) {
  ...
  if (!check_time_update(msg->timestamp, robot_control_state_.parastamped.timestamp)) {
    message_error(TAG_PARAM + std::string("Timestamp is old. Return"));
    return;
  }
#endif

  if (msg->body_height == 0 || msg->gait_height == 0) {
    message_error(TAG_PARAM + std::string("Got zero in paras! Return"));
    return;
  } else {
    message_info(
        TAG_PARAM + std::string("Body height is ") + std::to_string(msg->body_height) +
        std::string(", gait height is ") + std::to_string(msg->gait_height));
    robot_control_state_.parastamped = *msg;
    publish_paras(robot_control_state_.parastamped);
  }
}

void MotionManager::caution_callback(const NavCaution_T::SharedPtr msg) { nav_caution_ = *msg; }
...
void MotionManager::control_state_callback(const ControlState_T::SharedPtr msg) {
  ...
  if (check_motor_errflag() ||                                // passive when motor error
      (msg->modestamped.control_mode == Mode_T::MODE_LOCK &&  // Lock Detection
       msg->gaitstamped.gait != Gait_T::GAIT_PASSIVE && !gait_server_->is_running() &&
       !mode_server_->is_running()))                          // avoid mode trigger gait
  {
    Gait_T passive_gait;
    passive_gait.gait = Gait_T::GAIT_PASSIVE;
    passive_gait.timestamp = this->get_clock()->now();
    message_info(
        TAG_STATE + std::string("Locking state & Error state detected. Checking to passive"));
...
void MotionManager::guard_callback(const Safety_T::SharedPtr msg) {
  ...
  if (robot_control_state_.safety != *msg) {
    if (msg->status == Safety_T::LOW_BTR) {
      ...
      message_info(TAG_GUARD + std::string("Battery low, reset max velocity"));
    }
    if (msg->status == Safety_T::NORMAL) {
      ...
      cons_abs_ang_r_ /= scale_low_btr_;
      cons_abs_ang_y_ /= scale_low_btr_;
      message_info(TAG_GUARD + std::string("Battery recovery, reset max velocity"));
    }
  }
  ...
  auto condition_timeout =
      this->get_clock()->now() - last_motion_time_ >= std::chrono::milliseconds(timeout_motion_);
...
void MotionManager::control_lcm_collection(
  ...
  if (response_count_ >= std::ceil(rate_lcm_const_ / rate_output_) - 1 && rclcpp::ok() &&
      thread_flag_) {
    publish_control_state(robot_control_state_);
    response_count_ = 0;
  }
...
void MotionManager::statees_lcm_collection(
  ...
  Time_T odom_stamp;
  odom_stamp.sec = std::chrono::duration_cast<std::chrono::seconds>(lcm_time).count();
  odom_stamp.nanosec =
      (lcm_time - std::chrono::duration_cast<std::chrono::seconds>(lcm_time)).count();
  ...
  if (odom_count_ >= std::ceil(rate_lcm_const_ / rate_odom_) - 1 && rclcpp::ok() && thread_flag_) {
...
void MotionManager::publish_velocity(
    const SE3VelocityCMD_T::SharedPtr velocity_out, const int8_t ORDER_TYPE) {
  auto TAG_VCMD = std::string("[VCMD_Publish]");
  if (source_id_map_.find(velocity_out->sourceid) == source_id_map_.end()) {
    message_warn(
        TAG_VCMD + std::string("Source id [") + std::to_string(velocity_out->sourceid) +
  ...
  if (check_time_update(velocity_out->velocity.timestamp, last_motion_time_)) {
    ...
    message_warn(TAG_VCMD + std::string("TimeStamp is invalid. Ignore it"));
    return;
  }
#else
  ...
  if (robot_control_state_.modestamped.control_mode < Mode_T::MODE_MANUAL) {
    message_warn(
        TAG_VCMD + std::string("Robot ignores all velocity command in current mode : ") +
        mode_label_[robot_control_state_.modestamped.control_mode]);
    return;
  }
  if (velocity_out->velocity.frameid.id != FrameID_T::BODY_FRAME) {
    message_warn(TAG_VCMD + std::string("Frame ID is not [BODY_FRAME], Ignore it"));
    return;
  }

  if (cmd_pub_->is_activated() && this->count_subscribers(cmd_pub_->get_topic_name()) > 0) {
    cmd_pub_->publish(*velocity_out);
  }

  ros_to_lcm_data_.linear[0] =
      limit_data(velocity_out->velocity.linear_x, 0 - cons_abs_lin_x_, cons_abs_lin_x_);
  ros_to_lcm_data_.linear[1] =
      limit_data(velocity_out->velocity.linear_y, 0 - cons_abs_lin_y_, cons_abs_lin_y_);
  ros_to_lcm_data_.linear[2] =
      limit_data(velocity_out->velocity.linear_z, 0 - cons_abs_lin_x_, cons_abs_lin_x_);

  ros_to_lcm_data_.angular[0] =
      limit_data(velocity_out->velocity.angular_x, 0 - cons_abs_ang_r_, cons_abs_ang_r_);
  ros_to_lcm_data_.angular[1] =
      limit_data(velocity_out->velocity.angular_y, 0 - cons_abs_ang_p_, cons_abs_ang_p_);
  ros_to_lcm_data_.angular[2] =
      limit_data(velocity_out->velocity.angular_z, 0 - cons_abs_ang_y_, cons_abs_ang_y_);
...
void MotionManager::init() {
  ...
  automation_manager_node_ = std::make_shared<cyberdog_decision::AutomationManager>("multi");
  automation_node_thread_ =
      std::make_unique<std::thread>(&MotionManager::automation_node_spin, this);
  lcm_control_res_handle_thread_ =
      std::make_unique<std::thread>(&MotionManager::recv_lcm_control_handle, this);
  lcm_statees_res_handle_thread_ =
      std::make_unique<std::thread>(&MotionManager::recv_lcm_statees_handle, this);
  control_cmd_thread_ = std::make_unique<std::thread>(&MotionManager::control_cmd_spin, this);
...
bool MotionManager::check_time_update(Time_T coming_time, Time_T stash_time) {
  if (coming_time.sec > stash_time.sec ||
      (coming_time.sec == stash_time.sec && coming_time.nanosec >= stash_time.nanosec)) {
    return true;
  } else {
    return false;
  }
...
bool MotionManager::inter_check_gait_sync(
    const Gait_T &goal_gait, const uint32_t &timeout, const uint8_t &priority) {
  auto goal = ChangeGait_T::Goal();
  bool rtn_(false);
...
void MotionManager::publish_odom(const nav_msgs::msg::Odometry &odom_out) {
  if (odom_pub_->is_activated() && this->count_subscribers(odom_pub_->get_topic_name()) > 0) {
  ...
  */
    ...
    if (robot_control_state_.modestamped.control_mode >= Mode_T::MODE_MANUAL &&
      ...
      if (is_zeros &&
        ...
        gait_pub_->publish(gait_out);
        message_info(TAG_MOVEMENT + std::string("Stop when zero"));
        ...
        message_info(TAG_MOVEMENT + std::string("Clear velocity cache"));
      } else if (!is_zeros) {
        message_info(
            TAG_MOVEMENT + std::string("Run with cached gait ") + gait_label_[gait_cached_.gait]);
...
void MotionManager::parameter_check(
    const T &extreme_value,
    T &value_to_check,
    const std::string &value_name,
    const bool &max_check) {
  auto condition_extreme =
      max_check ? (value_to_check > extreme_value) : (value_to_check < extreme_value);
  auto string_extreme = max_check ? std::string("is bigger than extreme value ")
                                  : std::string("is smaller than extreme value ");
  if (condition_extreme) {
    message_warn(std::string("Value ") + value_name + std::string(" invalid. Reset it"));
    ...
    if (isError || show_all) {
      ...
      for (int a = 0; a < motor_num; a++) {
        ...
        if (motor == 0) {
          ...
          motor >>= 1;
          if (motor & 0x01) {
            err_T += "\n\t|\tCoil Thermal-Shutdown";
          }
          motor >>= 1;
          if (motor & 0x01) {
            err_T += "\n\t|\tMotor driver chip error";
          }
          motor >>= 1;
          if (motor & 0x01) {
            err_T += "\n\t|\tMotor bus under voltage (< 10V)";
          }
          motor >>= 1;
          if (motor & 0x01) {
            err_T += "\n\t|\tMotor bus over voltage (> 30V)";
          }
          motor >>= 1;
          if (motor & 0x01) {
            err_T += "\n\t|\tB phase sample overcurrent (> 78A)";
          }
          motor >>= 1;
          if (motor & 0x01) {
            err_T += "\n\t|\tC phase sample overcurrent (> 78A)";
          }
          motor >>= 2;
          if (motor & 0x01) {
            err_T += "\n\t|\tEncoder not calibrate";
          }
          motor >>= 1;
          for (int n = 1; n <= 8; n++) {
            if (motor & 0x01) {
              err_T += "\n\t|\tOverload error " + std::to_string(n);
            }
            motor >>= 1;
          }
          if (motor & 0x01) {
            err_T += "\n\t|\tA phase sample overcurrent (> 78A)";
          }
          motor >>= 7;
          if (motor & 0x01) {
          ...
          motor >>= 1;
          if (motor & 0x01) {
            err_T += "\n\t|\tHall sensor calibrate error";
          }
          motor >>= 1;
          if (motor & 0x01) {
            err_T += "\n\t|\tTorque calibration data illegal";
          }
          motor >>= 1;
          if (motor & 0x01) {
